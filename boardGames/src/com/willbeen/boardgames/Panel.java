...
//	private JButton btnLoadGame;
	...
	public Panel(Game game, Board board) {
		board = (Board)new EnglishDraughtBoard();
	}
	
	public void setButtons(Game game, ButtonDescription[] buttonDescriptions) {
		int i = 0;
		actionButtons = new JButton[buttonDescriptions.length];
		for(ButtonDescription bd : buttonDescriptions) {
			actionButtons[i] = new JButton(bd.getText());
			actionButtons[i].setActionCommand(bd.getAction());
			...
			actionButtons[i].addActionListener(game);
			add(actionButtons[i]);
			i++;
		}
	}
